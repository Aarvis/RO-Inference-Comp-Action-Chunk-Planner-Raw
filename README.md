# RO-Inference-Comp-Action-Chunk-Planner-Raw

Standalone integrated inference package for both
`pi05_origami_comp_action_chunk` and
`pi05_origami_comp_action_chunk_phase2`. It does not modify or depend on the
existing `RO-Inference-Comp-Action-Chunk` project at runtime.
...
Read [configuration alignment](README_CONFIGURATION_ALIGNMENT.md) before
changing checkpoints, model settings, action horizon, or action stride.
...
```bash
cd /home/ubuntu/RO-Inference-Comp-Action-Chunk-Planner-Raw
...
docker build --build-arg EXECUTION_MODE=async -f ./docker/submission-zenoh-bundled.Dockerfile -t ro-inference-comp-action-chunk2:async .
```
...
ORIGAMI_MODEL_BUNDLE=/app/RO-Inference-Comp-Action-Chunk-Planner-Raw/model_bundle
...
Keep the image tag or digest, archive, checksum, expected horizon `10`, and the runtime resource assumptions used during validation.
