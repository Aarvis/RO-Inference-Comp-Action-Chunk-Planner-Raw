...
def _parse_origami_comp_action_chunk_config(
    *,
    ...
    server = ServerConfig(
        ...
        action_chunk_stride=int(server_cfg.get("action_chunk_stride", replay.action_chunk_stride)),
    ...
    if server.action_horizon < 1 or server.action_chunk_stride < 1:
        raise ValueError("server.action_horizon and action_chunk_stride must both be positive.")
    if (
        server.action_horizon != replay.action_horizon
        or server.action_chunk_stride != replay.action_chunk_stride
    ):
        raise ValueError(
            "server and dataset_replay must use the same action_horizon and action_chunk_stride."
        )
