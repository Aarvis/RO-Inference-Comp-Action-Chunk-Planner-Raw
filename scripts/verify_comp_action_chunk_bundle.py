...
def verify_bundle(
    ...
    *,
    ...
    if snapshot_stride != config.server.action_chunk_stride:
        raise ValueError(
            "OpenPI action_chunk_stride does not match server.action_chunk_stride: "
            f"{snapshot_stride} != {config.server.action_chunk_stride}"
        )
...
def main() -> int:
    ...
    parser.add_argument(
        "--allow-external-code",
        action="store_true",
        help="Allow OpenPI source to resolve outside RO-Inference-Comp-Action-Chunk-Planner-Raw/vendor/openpi/src.",
    )
    parser.add_argument(
